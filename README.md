...
| Superseded A/B fingerprint revision 2 | Host tools can identify and inspect this returned-card format read-only, including its existing logs. Its older selector manifest, zero `BPB_HiddSec` fields, and raw-slot journal digest are incompatible with the current revision-3 kernel/update contract. Recovery requires a deliberately targeted whole-card reflash; copying files is not an upgrade. |
