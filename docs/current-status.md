...
## Implemented GPU architecture foundations
...
The following foundations compile in the guest and have host tests, but are not
wired into the live boot renderer:
...
There is no accelerated boot marker or screenshot yet. The next honest crossing
is a fenced VirtIO/VirGL context and render target whose visible clear is
generated by the GPU, followed by lowering the shared scene command stream.
...
not a Raspberry Pi or accelerated 3D graphics claim.
...
- a live VirtIO/VirGL GPU execution session and scene lowering, native BCM2712
  V3D VII MMU/command submission, HVS display lists and IOMMU integration,
...
The next graphics milestone is driver-first: wire a bounded VirtIO/VirGL
session, prove a fenced GPU-generated clear, lower the shared retained commands,
and move frame ownership through the graphics-worker mailbox. The Pi backend
then needs native V3D VII command submission and address translation plus HVS,
vblank, HDMI, HPD/DDC/EDID, clock, and PHY drivers behind the same contracts;
simplefb remains only a bring-up diagnostic. Input, block storage, entropy, and
networking drivers remain parallel work. The proof scheduler still needs per-
CPU state and interrupt interfaces, runnable work on secondaries, a versioned
syscall ABI, executable loading, and a minimal VFS before the renderer can become
a user-facing compositor service.
